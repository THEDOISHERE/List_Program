...
function addProduct() {
  ...
  if (product.value === "") {
    ...
    //let output = document.getElementById("divis");
  }

  //divis = document.querySelector("divis");
  // Assignment 2:
  ...
  // this line is to prevent the webpage from reloadings. DO NOT MODIFY IT.

  document.getElementById(productArray.value) === "";
  output.style.display = "flex";

  const divis = document.createElement("divis");
  document.body.appendChild(divis);
  for (let i = 0; i < productArray.length; i++) {
    console.log(productArray.length);
    console.log(productArray[i]);
    //divis.innerHTML = product.value;
    divis.innerHTML = `${productArray[i]}`;
    console.log(productArray[i].product);
    output.appendChild(divis);
  }

  // for (let arrays of productArray) {
  //   divis.textContent = `${productArray[i].product}`;
  //   output.appendChild(divis);
  // }
